...
6. Member-owned originals stay unchanged. Lead writes its own reviewable consolidated draft and sends that exact version plus original evidence to Evidence for substantive independent review and the formal six-pack CLI. Each Evidence TaskSpec must name one exact output path in `delivery_requirements` and include it in `callee_write_paths`; a file grant never authorizes a sibling path. Corrections return to each responsible author’s own directory as new versions, including Lead’s draft. Evidence actually rechecks the final version; Lead binds delivery to its exact files/hashes, authors and real final receipts. Before any completion conclusion, Lead must actually read and hash the designated Evidence delivery file, verify it names the exact final-pack identity and records a real validator exit 0. A child status, source-read claim, tool transcript, chat reply, or empty output directory is not an Evidence receipt. Missing, unparsable, hash-mismatched, partial, blocked, or failed Evidence delivery forces `partial_only` or `cannot_conclude`, with the missing artifact and reason listed; it may never be represented as `pass` or `pass_with_cautions`. Lead must not run Evidence’s venv, replace independent review, or describe an earlier failing receipt as a later pass. Missing/unresolved proof remains partial/unverified; restoring an edited file does not erase a write.
