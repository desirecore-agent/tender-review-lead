...
失败仍退出2，不返回成功manifest，并保留可能已创建的部分输出。stderr是JSON对象，保留 `code: PREPARATION_REFUSED` 与 `partial_output_policy: preserved_if_created`，新增 `reason_code` 和 `slot`。新增字段只使用固定标签，不含路径、请求正文、JSON字段内容、摘要或异常文本。CLI语法/选项错误返回 `ARGUMENTS_INVALID` / `arguments`，不回显调用者输入；正常 `--help` 仍可用。

示例：`PATH_NOT_FOUND` / `authority` 表示明确指定的authority输入无法打开；`HASH_MISMATCH` / `request` 表示请求与预期摘要不同；`ASSIGNMENT_SPEC_MISMATCH` / `assignment` 表示与Spec共享字段不一致；`OUTPUT_EXISTS` / `output_task` 或 `output_manifest` 表示该输出已存在。`ACCESS_REFUSED`、`UNSAFE_PATH` 不授权改权限或跟随链接。解析失败使用 `INVALID_JSON`、`INVALID_TEXT`、`JSON_LIMIT` 或 `INVALID_STRUCTURE`；其它I/O错误使用 `IO_ERROR`。已有语义拒绝标签通过 `reason_code` 保留。

输入槽位为 `request`、`assignment`、`spec`、`authority`、`preflight`、`execution_receipt`；其它槽位为 `arguments`、`target_agent_id`、`kind`、`role`、`output`、`output_name`、`output_directory`、`output_task`、`output_manifest`。槽位仅指出失败检查上下文，不证明其余输入均合法。只能根据真实证据修正已授权输入/身份；禁止为消除报错而制造Spec、authority、pass或hash，禁止覆盖部分输出或将其当成功。本helper的256KiB限制、排他创建、源/输出身份复验与成功输出字节均不变。
